...
torchrun --standalone --nproc_per_node=3 gpt2.py \
    > gpt2.log 2>&1
"""
...
class DataLoaderLite:
  def __init__(self, B, T, process_rank, num_processes, split):
    ...
    data_root = "/DATA405/khushi/gpt/edu_fineweb10B"
...
# create log directory to save checkpoints to
log_dir = "log"
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(log_dir, f"tog.txt")
with open(log_file, "w") as f: # open for writing into it
   pass


for step in range(max_steps):
  loss_accum = torch.zeros((), device=device)  # Tensor on GPU
  t0 = time.time()
  ...
  if master_process:
    print(f"step : {step} | loss : {loss_accum.item():.6f} | tok/sec : {tokens_per_sec} | lr : {lr:.6e} | dt : {dt:.2f}s")
    with open(log_file, "a") as f:
...
# eval set : HellaSwag - 2019 - its a MCQ dataset, sentence completions
# Seqence length = length of largest option and others are padded
# Batch size = 4 (1 MCQ : Question witrh 4 answers)

# GPT-3 has ~33.5% accuracy on HellaSwag dataset
# we are training on 10B tokens while gpt2 was trained on 100B tokens